...
import { Mic, PhoneOff, Bot, User } from 'lucide-react'
      ...
      {multipliers.map((m, i) => {
        ...
        return (
          <div
            ...
            style={{
              transform: `scaleY(${Math.max(0.12, Math.min(1, h))})`,
...
function useSyntheticWave(isSpeaking) {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      frame++
      const v = 0.5 + 0.4 * Math.sin(frame / 10) + 0.1 * Math.sin(frame / 2.3)
...
export default function VoiceAssistantPage() {
  ...
  const userAvatarClass = [
    'h-12 w-12 rounded-full overflow-hidden border border-border bg-emerald-500/10',
    userSpeaking
          ...
          <div className="container mx-auto max-w-6xl px-4">
            <Card className="bg-card/90 backdrop-blur-md border border-border/70 overflow-hidden relative">
              ...
              <CardContent className="p-6 space-y-6">
                {/* AI + USER PANELS */}
                <div className="flex flex-col md:flex-row md:items-stretch md:gap-4 gap-4">
                  {/* EstageGo AI */}
                  ...
                  >
                    {history.length === 0 && !liveTurn ? (
                      <div className="h-full w-full min-h-50 flex items-center justify-center text-muted-foreground text-center px-6 text-xs">
                        Nội dung cuộc gọi sẽ hiển thị ở đây
                      </div>
                    ) : (
                      <div className="space-y-3">
                            ...
                            >
                              ...
                              >
                                <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1 font-mono flex items-center gap-2">
                                  <span>{isAI ? 'EstageGo AI' : 'Bạn'}</span>
                                </div>
                        ...
                        {liveTurn ? (
                          ...
                          >
                            ...
                            >
                              <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1 font-mono flex items-center gap-2">
                                <span>{liveTurn.role === 'assistant' ? 'EstageGo AI' : 'Bạn'}</span>
                                <span className="relative flex h-2 w-2">
